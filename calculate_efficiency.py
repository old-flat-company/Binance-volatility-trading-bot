...
def calculate_efficiency():
    ...
    STANDARD_COEFF = 3 / 1
    MIN_NUM_RESULTS = 4
    now_unix_time = time.mktime(datetime.now().timetuple())
    # int(line_data.strip().split('\t')[0])  -- unix time
    # last 1 hour  data
    last_data_list = [line_data for line_data in open("efficiency_log.txt", 'r')
                      if now_unix_time - int(line_data.strip().split('\t')[0]) <= CHECKING_TIME]
    ...
    for last_data in last_data_list:
        curr_unix_time, curr_res = last_data.strip().split('\t')
        if curr_res == '+':
            positive_res += 1
        else:
            negative_res += 1
